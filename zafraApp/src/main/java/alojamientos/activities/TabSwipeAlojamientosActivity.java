package alojamientos.activities;
...
import alojamientos.adapters.TabsAlojamientosPager;
import ayto.zafrApp.R;
import clases_principales.MenuPrincipalActivity;

public class TabSwipeAlojamientosActivity extends AppCompatActivity implements
...
private TabsAlojamientosPager mAdapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Db_Adapter = new DbAlojamientosAdapter(this);
		Db_Adapter.abrir(); 
		...
		if (extra.containsKey(DbAlojamientosAdapter.C_ALO_COLUMNA_ID))
		{
			id = extra.getLong(DbAlojamientosAdapter.C_ALO_COLUMNA_ID);
		}
		...
		mAdapter = new TabsAlojamientosPager(getSupportFragmentManager(),bundle);
...
public boolean onOptionsItemSelected(MenuItem item){
	...
	String titulo=(cursor.getString(cursor.getColumnIndex(DbAlojamientosAdapter.C_ALO_COLUMNA_NOMBRE)));
...
public void VerGPS(View v)
{
	...
	cursor = Db_Adapter.getRegistro(id);
  final Double latgps = cursor.getDouble(cursor.getColumnIndex(DbAlojamientosAdapter.C_ALO_COLUMNA_GPSLATITUD));
  final Double longps = cursor.getDouble(cursor.getColumnIndex(DbAlojamientosAdapter.C_ALO_COLUMNA_GPSLONGITUD));
